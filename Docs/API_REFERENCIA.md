# MinusFramework — Referência da API Pública

> Interfaces, classes e métodos públicos do MinusORM v2.0
> **Última atualização:** Junho 2026 — Sprint 4 (ArrayDML)

---

## Conexão (`MF.Connection.pas`, `MF.Types.pas`)

### `IConexao`

A interface central de abstração do banco de dados.
...
IComando = interface
  ...
  function Executar: Integer;          // INSERT/UPDATE/DELETE → linhas afetadas
  function ExecutarConsulta: IResultados; // SELECT → resultset
  procedure Cancelar;
  // Array DML — execução em lote com 1 round-trip (Sprint 4)
...
IParametro = interface
  ...
  procedure Limpar;
  // Array DML — valores por índice (Sprint 4)
  procedure DefinirArraySize(ASize: Integer);
...
IResultados = interface
  function Proximo: Boolean;           // Avança; retorna False se não há mais registros
...
## Configuração (`MF.Config.pas`)

### `TConfiguracaoORM`

Gerencia conexões nomeadas e cache de segundo nível.
...
Gerencia transações e filas de CRUD com Change Tracking.
...
procedure Confirmar;    // DELETE → INSERT → UPDATE em uma transação
...
**Fluxo do `Confirmar`:**
1. `IniciarTransacao`
2. Executa DELETE para entidades excluídas (respeita SoftDelete)
...
**Métodos virtuais (override para customização):**
```pascal
function ObterOrdenacao: string; virtual;           // ORDER BY padrão
procedure AntesExcluir(const AIdentificador: Integer); virtual;
procedure AposBuscarPorId(const AEntidade: T); virtual;
```

**Observação:** `TRepositorioBase<T>.Salvar` lida com ciclo completo:
- Se `Id = 0` → INSERT (com ou sem RETURNING, conforme `IGeradorId.SuportaRetorno`)
- Se `Id > 0` → UPDATE (com validação de versão e `EErroConcorrencia`)
- Valida `[NotNull]`, `[ChaveUnica]`, dispara shadow properties e auditoria

### `TMapaIdentidade` (`MF.IdentityMap.pas`)

Cache de primeiro nível por classe + ID.
...
Detecta mudanças por snapshot de RTTI.
...
| `[ChavePrimaria]` | Propriedade | Identificador único |
| `[Ignorar]` | Propriedade | Campo não persistente |
| `[NotNull]` | Propriedade | Validação de obrigatoriedade |
| `[ReadOnly]` | Propriedade | Excluído de INSERT/UPDATE |
| `[Versao('col', 1)]` | Propriedade | Lock otimista |
| `[Cache(ttl, 'regiao')]` | Classe | Cache de segundo nível |
| `[SoftDelete('col', tipo)]` | Classe | Exclusão lógica |
...
| `[CriadoEm]` | Propriedade | Shadow: setado automático no INSERT |
| `[AtualizadoEm]` | Propriedade | Shadow: setado automático no INSERT/UPDATE |
...
- `bdFirebird` → `TGeradorIdFirebird` (GEN_GEN_TABELA_ID via RETURNING)
- `bdPostgreSQL` → `TGeradorIdPostgreSQL` (NEXTVAL via RETURNING)
- `bdSQLite` → `TGeradorIdSQLite` (last_insert_rowid())
- `bdMySQL` → `TGeradorIdMySQL` (LAST_INSERT_ID())

---

## Exceções (`MF.Exceptions.pas`)
...
O framework inclui mocks reutilizáveis para testes unitários sem banco de dados:

| Mock | Implementa | Uso |
|---|---|---|
| `TConexaoMock` | `IConexao` | Rastreia `IniciarTransacao`, `Confirmar`, `Reverter`; retorna `TComandoMock` |
| `TComandoMock` | `IComando` | Rastreia SQL e parâmetros; `ExecutarResult` e `ExecutarConsultaResult` configuráveis |
